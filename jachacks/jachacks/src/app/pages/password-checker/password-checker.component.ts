...
import {
  PasswordAnalyseState,
  PasswordAnalysis,
} from '../../types/passwordAnalyse';
...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, NgClass, InputComponent, ButtonComponent],
...
export class PasswordCheckerComponent implements OnInit {
  ...
  async makeApiCalls() {
    ...
    this.analysis = await this.passwordCheckerService.checkPassword(
      this.password
    );
    this.transform =
      this.availableWidth[
        isNaN(this.analysis.gptDegreeOfDanger)
          ? 9
          : this.analysis.gptDegreeOfDanger
      ];
