...
import {
  PhishingAnalyseMode,
  PhishingAnalyseState,
} from '../../../types/PhishingAnalyse';
...
@Component({
  ...
  standalone: true,
  imports: [InputComponent, FormsModule, NgClass, ButtonComponent],
...
export class PhishingDetectComponent {
  ...
  private async detect(): Promise<void> {
    this.res = await this.phishingDetectorService.detectPhishing(
      this.from,
      this.to,
      this.subject,
      this.body
    ...
    this.transform =
      this.availableWidth[isNaN(this.res.score) ? 9 : this.res.score * 2 - 1];
  }

  public getResReasoning = () => {
    if (this.res) {
      return this.res.reasonning
        .map((el) => el.split('-'))
        .flat()
        .map((el) => el.split('\n'))
        .flat()
  ...
  private detectFromFile = async (): Promise<void> => {
    return new Promise<void>((resolve) => {
      ...
      reader.onload = async () => {
        let res = await this.phishingDetectorService.detectPhishingFromSMTP(
        ...
        if (typeof res === 'string') {
          ...
          this.transform =
            this.availableWidth[
              isNaN(this.res.score) ? 9 : this.res.score * 2 - 1
            ];
          this.fileError = '';
        }
        resolve();
